...
def iterative_deepening(board, max_time=TIME_LIMIT):
    ...
    for depth in range(1, MAX_DEPTH + 1):
        ...
        if STATS:
            ...
            print(f"Depth: {depth}, Score: {best_score}, Time: {depth_time:.2f}s, "
...
def play_game():
    print("NegaKnightV7 - Enhanced Positional Chess Engine")
    print("Enter moves in UCI (e2e4) or SAN (Nf3) format")
    color = input("Play as (w)hite or (b)lack? ").lower()
    player_white = color.startswith('w')
    board = chess.Board()
    game = chess.pgn.Game()
    game.headers["White"] = "Human" if player_white else "NegaKnightV7"
    game.headers["Black"] = "NegaKnightV7" if player_white else "Human"
    game.headers["Date"] = datetime.now().strftime("%Y.%m.%d")
    node = game
    while not board.is_game_over():
        print_board(board)
        if (board.turn == chess.WHITE) == player_white:
            move = get_move(board)
            if move is None:
                print("Game quit!")
                return
